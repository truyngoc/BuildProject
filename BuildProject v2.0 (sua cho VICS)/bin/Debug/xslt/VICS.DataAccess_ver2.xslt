...
<xsl:template match="entity/columns" mode="Insert">
        ...
        defaultDB.AddInParameter(dbCommand,"p_<xsl:value-of select="@columnName"/>", DbType.<xsl:value-of select="@oracle2DBType"/>, GetDBNull(obj.<xsl:value-of select="@columnName"/>) _
		</xsl:for-each>
</xsl:template>
